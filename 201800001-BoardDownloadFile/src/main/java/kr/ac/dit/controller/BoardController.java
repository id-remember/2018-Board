...
public class BoardController {
	...
	public void readGET(@RequestParam("no") int no, Model model) throws Exception {
		...
		System.out.println(boardService.readAttachFile(no));
	}
	...
	public ResponseEntity<byte[]> download(@RequestParam("fileName") String fileName) throws Exception {
		...
		try {
			...
			httpHeaders.add("Content-Disposition", "attatchment; filename=\"" + 
					new String(fileName.getBytes("UTF-8"), "ISO-8859-1") + "\"");			responseEntity = new ResponseEntity<byte[]>(IOUtils.toByteArray(inputStream), httpHeaders, HttpStatus.CREATED);
		} catch(Exception e) {
			e.printStackTrace();
		} finally {
			inputStream.close();
